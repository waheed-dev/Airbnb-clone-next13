...
const Input : React.FC<InputProps> = ({errors,id,disabled,formatPrice,register,type = 'text',required,label}) => {
    return (
        <div className={'w-full relative'}>
            ...
            type={type} className={`peer w-full p-4 pt-5 font-light bg-white border-2 rounded-md transition outline-none disabled:opacity-70 disabled:cursor-not-allowed ${formatPrice ? 'pl-9' : 'pl-4'} ${errors[id] ? 'border-rose-500' : 'border-neutral-300'}  ${errors[id] ? 'focus:border-rose-500' : 'focus:border-black'}`}
            />
